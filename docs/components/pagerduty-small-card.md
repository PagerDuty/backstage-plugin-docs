...
This card doesn't show any *incidents* or *recent changes* as the idea is to be able to quickly check the service status and use as little space on the screen as possible. For this reason, we moved the *insights* and *on call* sections into collapsible panels that can be expanded when needed.
...
You can optionally also disable the ability to *create new incidents* from the card by making it `read-only`. Refer to the [documentation](/backstage-plugin-docs/advanced/enable-read-only-mode) to see how to do it.
